...
number_of_repeating_unit = 9
ratio_of_main_unit = 0.5
inverse = False

main_molecule = Chem.MolFromMolFile('monomer.mol', sanitize=False)
sub_molecule = Chem.MolFromMolFile('monomer_pet.mol', sanitize=False)
main_molecule_smi = Chem.MolToSmiles(main_molecule)
sub_molecule_smi = Chem.MolToSmiles(sub_molecule)
...
for repeat in range(number_of_repeating_unit - 1):
    if repeat == 0:
        ...
        # make adjacency matrix and get atoms for main molecule
        sub_adjacency_matrix = Chem.rdmolops.GetAdjacencyMatrix(sub_molecule)
        for bond in sub_molecule.GetBonds():
            sub_adjacency_matrix[bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()] = bond_list.index(bond.GetBondType())
            sub_adjacency_matrix[bond.GetEndAtomIdx(), bond.GetBeginAtomIdx()] = bond_list.index(bond.GetBondType())
        sub_atoms = []
        for atom in sub_molecule.GetAtoms():
            sub_atoms.append(atom.GetSymbol())

        r_index_in_sub_molecule_old = [index for index, atom in enumerate(sub_atoms) if atom == '*']
        ...
        added_atoms_roop = main_atoms.copy()
        added_adjacency_matrix_roop = main_adjacency_matrix.copy()
        r_index_in_added_molecule_old_roop = r_index_in_main_molecule_old.copy()
        ...
        if random_number <= ratio_of_main_unit: #Main
            if inverse:
        ...
        if random_number > ratio_of_main_unit: #Sub
            added_atoms = sub_atoms.copy()
            added_adjacency_matrix = sub_adjacency_matrix.copy()
            r_index_in_added_molecule_old = r_index_in_sub_molecule_old.copy()
        ...
        #どちらかを選ぶ
        random_number = random.random()
        if  random_number <= ratio_of_main_unit:

            added_atoms = added_atoms_roop.copy()
            added_adjacency_matrix = added_adjacency_matrix_roop.copy()
            r_index_in_added_molecule_old = r_index_in_main_molecule_old.copy()
        ...
        elif random_number >  ratio_of_main_unit:
            # make adjacency matrix and get atoms for main molecule
            sub_adjacency_matrix_roop = Chem.rdmolops.GetAdjacencyMatrix(sub_molecule)
            for bond in sub_molecule.GetBonds():
                sub_adjacency_matrix_roop[bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()] = bond_list.index(bond.GetBondType())
                sub_adjacency_matrix_roop[bond.GetEndAtomIdx(), bond.GetBeginAtomIdx()] = bond_list.index(bond.GetBondType())
            sub_atoms_roop = []
            for atom in sub_molecule.GetAtoms():
                sub_atoms_roop.append(atom.GetSymbol())

            r_index_in_sub_molecule_old = [index for index, atom in enumerate(sub_atoms_roop) if atom == '*']

            added_atoms = sub_atoms_roop.copy()
            added_adjacency_matrix = sub_adjacency_matrix_roop.copy()
            r_index_in_added_molecule_old = r_index_in_sub_molecule_old.copy()
...
print(generated_molecule_smi)
Draw.MolToImage(generated_molecule)
